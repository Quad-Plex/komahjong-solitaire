...
- **US-03 (done):** 42 tile kinds + 144-tile deck + flower/season match rule in `mahjonglogic.lua`
  ...
  per tile; symbols are large with little inner padding. `installIconsIfNeeded()` copies the SVGs
  with a pure-Lua `io.open` byte copy (P2 refactor — no per-file `os.execute('cp')` shell forks,
  which used to spawn ~45 processes at every plugin load).
