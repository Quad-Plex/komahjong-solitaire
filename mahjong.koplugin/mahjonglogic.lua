...
-- Returns the independently-rendered bevel segments visible around a tile.
-- Full-grid neighbours hide an entire edge. Half-grid neighbours hide only
-- their overlapping half, so the renderer can add/remove a small bevel piece
-- without replacing the tile face. Segment names are deliberately geometric:
-- right_top/right_bottom and bottom_left/bottom_right refer to the visible
-- halves of the corresponding outward bevel.
function MahjongLogic.visibleBevelSegments(board, x, y, layer)
    if not MahjongLogic.tileAt(board, x, y, layer) then return {} end

    local segments = {}
    local east_full = MahjongLogic.tileAt(board, x + 1, y, layer) ~= nil
    local east_top = MahjongLogic.tileAt(board, x + 1, y - 0.5, layer) ~= nil
    local east_bottom = MahjongLogic.tileAt(board, x + 1, y + 0.5, layer) ~= nil
    if not east_full then
        if not east_top and not east_bottom then
            segments[#segments + 1] = "right"
        elseif not east_top then
            segments[#segments + 1] = "right_top"
        elseif not east_bottom then
            segments[#segments + 1] = "right_bottom"
        end
    end

    local south_full = MahjongLogic.tileAt(board, x, y + 1, layer) ~= nil
    local south_left = MahjongLogic.tileAt(board, x - 0.5, y + 1, layer) ~= nil
    local south_right = MahjongLogic.tileAt(board, x + 0.5, y + 1, layer) ~= nil
    if not south_full then
        if not south_left and not south_right then
            segments[#segments + 1] = "bottom"
        elseif not south_left then
            segments[#segments + 1] = "bottom_left"
        elseif not south_right then
            segments[#segments + 1] = "bottom_right"
        end
    end
    return segments
end
...
function MahjongLogic.runSelfTests()
    ...
    -- Independently rendered bevel segments: a same-layer neighbour below hides
    -- the bottom edge, one to the right hides the right edge.
    local bv = boardWith{
        {2,2,0,"b1"}, {3,2,0,"b2"}, {2,3,0,"b3"}, {3,3,0,"b4"},
    }
    local function segments(board, x, y, layer)
        local result = MahjongLogic.visibleBevelSegments(board, x, y, layer)
        table.sort(result)
        return table.concat(result, ",")
    end
    check(segments(bv, 2, 2, 0) == "", "tile with right + bottom neighbours has no bevels")
    check(segments(bv, 3, 2, 0) == "right", "tile with only a bottom neighbour keeps right bevel")
    check(segments(bv, 2, 3, 0) == "bottom", "tile with only a right neighbour keeps bottom bevel")
    check(segments(bv, 3, 3, 0) == "bottom,right", "corner tile with no neighbours keeps both bevels")
    check(segments(boardWith{ {5,5,0,"east"} }, 5, 5, 0) == "bottom,right",
        "lone tile keeps both bevels")
    check(segments(bv, 9, 9, 0) == "", "no bevels for an empty cell")

    -- Half-grid protrusions: one half-neighbour leaves the complementary bevel
    -- segment visible; two half-neighbours remove the entire edge.
    local tur_bevel = MahjongLogic.newGame(42)
    check(segments(tur_bevel, 0, 3.5, 0) == "bottom",
        "head has no right bevel because both east halves are covered")
    check(segments(tur_bevel, 6.5, 3.5, 4) == "bottom,right",
        "cap (6.5,3.5,L4) keeps both bevels")
    check(segments(tur_bevel, 14, 3.5, 0) == "bottom,right",
        "east tail tip (14,3.5) keeps both bevels")
    check(segments(tur_bevel, 13, 3.5, 0) == "bottom",
        "left tail tile has no right bevel (neighbour (14,3.5) covers it)")
    -- (12,3) is only PARTIALLY covered on its east by the tail (which starts
    -- at y=3.5), so it keeps the exposed half of the right bevel.
    check(segments(tur_bevel, 12, 3, 0) == "right_top",
        "(12,3) keeps the exposed half of its right bevel")
    local half = boardWith{ {2, 2, 0, "b1"}, {3, 1.5, 0, "b2"} }
    check(segments(half, 2, 2, 0) == "bottom,right_bottom",
        "one upper east half-neighbour leaves the lower right bevel")
    local half_bottom = boardWith{ {2, 2, 0, "b1"}, {1.5, 3, 0, "b2"} }
    check(segments(half_bottom, 2, 2, 0) == "bottom_right,right",
        "one left south half-neighbour leaves the right bottom bevel")
